...
it("holds ChannelHeader geometry, colors, and optical alignment", { timeout: 90000 }, async () => {
    ...
    const header = (s: string) =>
        view.$(`[data-testid="${s}"] [data-happy-desktop-ui="channel-header"]`);
    const part = (s: string, name: string) =>
        view.$(`[data-testid="${s}"] [data-happy-desktop-ui="channel-header-${name}"]`);
    ...
    const starIcon = view.$(
        '[data-testid="s-full"] [data-happy-desktop-ui="channel-header-star"] [data-happy-desktop-ui="icon"]',
    );
    ...
    const membersIcon = view.$(
        '[data-testid="s-full"] [data-happy-desktop-ui="channel-header-members"] [data-happy-desktop-ui="icon"]',
    );
    ...
    /* ---- Agent chip ----------------------------------------------------- */
    const chip = view.$('[data-testid="s-full"] [data-happy-desktop-ui="badge"]');
    ...
    expect(chip.element.querySelector('[data-happy-desktop-ui="badge-label"]')?.textContent).toBe(
        "3 agents",
    );
    expect(chip.bounds().height).toBe(18);
    expect(
        count("s-full", '[data-happy-desktop-ui="badge-icon"] [data-name="spark"]'),
        "agent chip spark glyph",
    ).toBe(1);
    /* ---- Actions slot --------------------------------------------------- */
    expect(
        count("s-full", '[data-happy-desktop-ui="channel-header-actions"]'),
        "actions slot",
    ).toBe(1);
    expect(
        count(
            "s-full",
            '[data-happy-desktop-ui="channel-header-actions"] [data-happy-desktop-ui="button"]',
        ),
    ).toBe(2);
    /* ---- Overflow menu: closed → open on click -------------------------- */
    const menuButton = view.$(
        '[data-testid="s-full"] [data-happy-desktop-ui="channel-header-menu"] [data-happy-desktop-ui="button"]',
    );
    expect(menuButton.element.getAttribute("aria-haspopup")).toBe("menu");
    expect(
        count("s-full", '[data-happy-desktop-ui="channel-header-menu-popover"]'),
        "menu closed",
    ).toBe(0);
    ...
    expect(
        count(
            "s-full",
            '[data-happy-desktop-ui="channel-header-menu-popover"] [data-happy-desktop-ui="menu-item"]',
        ),
    ...
    const minIcon = view.$(
        '[data-testid="s-min"] [data-happy-desktop-ui="channel-header-icon"] [data-happy-desktop-ui="icon"]',
    );
    ...
    for (const name of ["star", "dot", "topic", "members", "actions", "menu"]) {
        expect(
            count("s-min", `[data-happy-desktop-ui="channel-header-${name}"]`),
            `minimal has no ${name}`,
        ).toBe(0);
    }
    expect(count("s-min", '[data-happy-desktop-ui="badge"]'), "minimal has no agent chip").toBe(0);
