...
it("centers Badge label and icon ink in every variant", { timeout: 120000 }, async () => {
    ...
    for (const variant of BADGE_VARIANTS) {
        ...
        const measured = await drift(
            `badge/${variant}`,
            view.$(`.v-${variant} [data-happy2-ui="badge-label"]`),
            badge,
        );
    ...
    const one = await drift(
        "badge/one-char",
        view.$('.v-one [data-happy2-ui="badge-label"]'),
        view.$(".v-one"),
    );
...
it(
    ...
    async () => {
        ...
        for (const entry of cases) {
            ...
            const badge = view.$(`.${id}`);
            const label = view.$(`.${id} [data-happy2-ui="count-badge-label"]`);
...
it(
    ...
    async () => {
        ...
        for (const [index, emoji] of cases.entries()) {
            ...
            const glyph = view.$(`.emoji-${index} [data-happy2-ui="reaction-chip-emoji-glyph"]`);
...
it(
    ...
    async () => {
        ...
        for (const count of counts) {
            ...
            const label = view.$(`.count-${count} [data-happy2-ui="reaction-chip-count"]`);
...
it(
    ...
    async () => {
        ...
        const innerOf = (cls: string, k: number, sym: boolean) =>
            view.$(
                `.${cls} [data-happy2-ui="key-cap-key"]:nth-child(${k + 1}) ${sym ? "svg" : '[data-happy2-ui="key-cap-text"]'}`,
            );
        ...
        for (const [cls, ch] of symCells) {
            ...
            const slot = view.$(`.${cls} [data-happy2-ui="key-cap-key"]`);
            const dx =
...
it("holds Badge family geometry, colors, and behavior", async () => {
    ...
    for (const variant of BADGE_VARIANTS) {
        ...
        expect(
            view.$(`.g-${variant} [data-happy2-ui="badge-label"]`).computedStyle("margin-right"),
            variant,
        ).toBe("-0.6px");
    }
    ...
    const label = view.$('.g-icon [data-happy2-ui="badge-label"]');
    ...
    expect(
        view
            .$('.g-chip [data-happy2-ui="reaction-chip-count"]')
            .computedStyles([
                "color",
                "font-family",
                "font-size",
    ...
    const emojiSlot = view.$('.g-chip [data-happy2-ui="reaction-chip-emoji"]');
