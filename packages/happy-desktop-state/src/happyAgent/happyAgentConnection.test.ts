...
import {
    happyAgentConnectionLoaderCreate,
    type HappyAgentConnectionLoaderOptions,
    type HappyAgentDaemonHealth,
} from "./happyAgentConnection.js";
...
function probeQueue() {
    const calls: Array<{
        resolve: (health: HappyAgentDaemonHealth) => void;
        reject: (error: unknown) => void;
    }> = [];
    const probe = (): Promise<HappyAgentDaemonHealth> =>
    ...
    return {
        ...
        async settle(
            index: number,
            result: HappyAgentDaemonHealth | { error: unknown },
        ): Promise<void> {
...
function loader(overrides: Partial<HappyAgentConnectionLoaderOptions> = {}) {
    const timers = timerHarness();
    const probe = probeQueue();
    const store = happyAgentConnectionLoaderCreate({
        probe: probe.probe,
        heartbeatMs: 2_000,
...
const ready: HappyAgentDaemonHealth = { status: "ready", version: "1.2.3" };

describe("happyAgentConnectionLoaderCreate", () => {
