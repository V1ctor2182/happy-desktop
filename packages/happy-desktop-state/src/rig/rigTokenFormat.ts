 ...
 */
export function rigTokenCountFormat(tokens: number): string {
...
export function rigCompactionSubject(
    estimatedTokensBefore: number,
    estimatedTokensAfter?: number,
): string {
    const before = rigTokenCountFormat(estimatedTokensBefore);
    return estimatedTokensAfter === undefined
        ? `from ${before} tokens`
        : `${before} → ${rigTokenCountFormat(estimatedTokensAfter)} tokens`;
}
