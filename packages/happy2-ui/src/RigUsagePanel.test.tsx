...
it("renders per-model token/cost rows, totals, context, and quota windows", async () => {
    ...
    const rows = view.container.querySelectorAll(
        '[data-testid="u"] [data-happy2-ui="rig-usage-group"]',
    );
    ...
    expect(
        view.$('[data-testid="u"] [data-happy2-ui="rig-usage-totals"]').element.textContent,
    ).toContain("307,500");

    // Context line reports approximate occupancy.
    expect(
        view.$('[data-testid="u"] [data-happy2-ui="rig-usage-context"]').element.textContent,
    ).toContain("approximate");

    // Two quota windows, the 93% one flagged full.
    const windows = view.container.querySelectorAll(
        '[data-testid="u"] [data-happy2-ui="rig-usage-quota-window"]',
    );
...
it("shows loading and error states without a snapshot", async () => {
    ...
    expect(
        view.container.querySelector('[data-testid="loading"] [data-happy2-ui="rig-usage-empty"]'),
    ).not.toBeNull();
    expect(
        view.$('[data-testid="error"] [data-happy2-ui="rig-usage-error"]').element.textContent,
