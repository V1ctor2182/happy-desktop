...
function question(overrides: Partial<RigUserInputQuestion> = {}): RigUserInputQuestion {
    return {
        header: "Migration",
        id: "q1",
        multiSelect: false,
...
function sourceItem(questions: readonly RigUserInputQuestion[]): RigInboxSourceItem {
    return {
        createdAt: 1,
        id: "one",
 ...
 */
function fixtureCreate(questions: readonly RigUserInputQuestion[] = [question()]) {
    const sent: RigInboxOutput[] = [];
    let emit = (_items: readonly RigInboxSourceItem[]): void => undefined;
...
describe("rig inbox written replies", () => {
