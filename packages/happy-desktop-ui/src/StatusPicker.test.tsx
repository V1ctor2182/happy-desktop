...
it("holds StatusPicker card, availability dots, segmented layout, and status field", async () => {
    ...
    // ---- Availability track --------------------------------------------
    const track = view.$(sp(' [data-happy-desktop-ui="status-picker-segmented"]'));
    ...
    for (const value of AVAILABILITY) {
        const seg = view.$(
            sp(` [data-availability="${value}"][data-happy-desktop-ui="status-picker-segment"]`),
        );
        ...
        const dot = view.$(
            sp(` [data-availability="${value}"][data-happy-desktop-ui="status-picker-dot"]`),
        );
        ...
        const label = view.$(
            sp(
                ` [data-availability="${value}"] [data-happy-desktop-ui="status-picker-segment-label"]`,
            ),
        );
    ...
    const onlineLabel = view.$(
        sp(' [data-availability="online"] [data-happy-desktop-ui="status-picker-segment-label"]'),
    );
    ...
    const pill = view.$(sp(' [data-happy-desktop-ui="status-picker-pill"]'));
    ...
    const availLabel = view.$(sp(' [data-happy-desktop-ui="status-picker-availability-label"]'));
    expect(
    ...
    // ---- Status field ---------------------------------------------------
    const field = view.$(sp(' [data-happy-desktop-ui="status-picker-field"]'));
    ...
    const emoji = view.$(sp(' [data-happy-desktop-ui="status-picker-emoji"]'));
    ...
    const input = view.$(sp(' [data-happy-desktop-ui="status-picker-input"]'));
    ...
    const clear = view.$(
        sp(' [data-happy-desktop-ui="status-picker-field"] [data-happy-desktop-ui="button"]'),
    );
    expect(clear.bounds(), "clear button box").toMatchObject({ width: 28, height: 28 });
    const clearGlyph = view.$(
        sp(
            ' [data-happy-desktop-ui="status-picker-field"] [data-happy-desktop-ui="button"] [data-happy-desktop-ui="icon"]',
        ),
    );
    ...
    // ---- Expiry meta ----------------------------------------------------
    const meta = view.$(sp(' [data-happy-desktop-ui="status-picker-meta"]'));
    expect(meta.computedStyle("color"), "meta color").toBe("rgb(73, 69, 79)");
    expect(
        view
            .$(
                sp(
                    ' [data-happy-desktop-ui="status-picker-meta-icon"] [data-happy-desktop-ui="icon"]',
                ),
            )
            .bounds(),
        "meta icon box",
    ...
    const availSection = view.$(sp(' [data-happy-desktop-ui="status-picker-availability"]'));
    const statusSection = view.$(sp(' [data-happy-desktop-ui="status-picker-status"]'));
    ...
    const statusLabelBounds = view
        .$(sp(' [data-happy-desktop-ui="status-picker-status-label"]'))
        .bounds();
...
it("holds StatusPicker availability sweep and empty-status state", async () => {
    ...
    for (const [index, value] of AVAILABILITY.entries()) {
        ...
        const pill = view.$(`${root} [data-happy-desktop-ui="status-picker-pill"]`);
        ...
        const selectedLabel = view.$(
            `${root} [data-availability="${value}"] [data-happy-desktop-ui="status-picker-segment-label"]`,
        );
        ...
        for (const other of AVAILABILITY) {
            ...
            const otherLabel = view.$(
                `${root} [data-availability="${other}"] [data-happy-desktop-ui="status-picker-segment-label"]`,
            );
            ...
            const otherDot = view.$(
                `${root} [data-availability="${other}"][data-happy-desktop-ui="status-picker-dot"]`,
            );
    ...
    const placeholderGlyph = view.$(
        '[data-testid="empty"] [data-happy-desktop-ui="status-picker-emoji"] [data-happy-desktop-ui="icon"]',
    );
    ...
    expect(
        emptyRoot.element.querySelector(
            '[data-happy-desktop-ui="status-picker-field"] [data-happy-desktop-ui="button"]',
        ),
        "no clear button when empty",
    ).toBe(null);
    expect(
        emptyRoot.element.querySelector('[data-happy-desktop-ui="status-picker-meta"]'),
        "no meta when no expiry",
    ).toBe(null);
    const emptyInput = view.$(
        '[data-testid="empty"] [data-happy-desktop-ui="status-picker-input"]',
    );
