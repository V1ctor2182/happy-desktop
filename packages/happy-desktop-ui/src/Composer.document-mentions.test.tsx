...
it("groups document mentions under their own subsection and reports selection", async () => {
    ...
    const rows = Array.from(
        document.querySelectorAll('[data-happy-desktop-ui="mention-picker-row"]'),
    );
    ...
    expect(
        rows[2]!.querySelector('[data-happy-desktop-ui="mention-picker-doc-glyph"]'),
    ).not.toBeNull();
    expect(rows[0]!.querySelector('[data-happy-desktop-ui="mention-picker-doc-glyph"]')).toBeNull();
    ...
    await userEvent.keyboard(" @");
    expect(
        document.querySelector('[data-happy-desktop-ui="mention-picker-documents-header"]'),
    ).not.toBeNull();
