...
export function DocumentEditor(props: DocumentEditorProps) {
    ...
    // eslint-disable-next-line happy2-react/no-layout-effect -- BlockNote retains these callbacks for the editor lifetime, so current host actions must update without recreating the editor or losing selection and undo state
    useLayoutEffect(() => {
    ...
    // eslint-disable-next-line happy2-react/no-layout-effect -- BlockNote exposes local document changes through an imperative subscription that must be paired with cleanup at the editor lifetime boundary
    useLayoutEffect(() => {
        const lifecycle = fileLifecycle.current;
    ...
    // eslint-disable-next-line happy2-react/no-layout-effect -- BlockNote's built-in generic file block has no open-affordance API, so its third-party DOM must be decorated and observed with complete cleanup
    useLayoutEffect(() => {
    ...
    // eslint-disable-next-line happy2-react/no-layout-effect -- Yjs Awareness is an imperative collaboration service whose event listener and instance require synchronous teardown
    useLayoutEffect(() => {
        const onUpdate = (
    ...
    // eslint-disable-next-line happy2-react/no-layout-effect -- Remote Yjs awareness updates enter through an imperative API and must be reconciled after the editor instance exists
    useLayoutEffect(() => {
