# Multiple Happy Agents: Network-Resilient Local and Remote Happy Agents
...
- The local host Happy Agent and every remote Happy Agent published by that host remain present
...
- Each affected Happy Agent reports `connecting`, `online`, `reconnecting`, `offline`,
  `error`, or `restricted` in context. A failure on one Happy Agent does not block
  another Happy Agent or local window-owned surfaces such as Notes and Blueprint.
...
- Operations requiring an answer from the unavailable Happy Agent are disabled with an
...
- Recovery reconciles durable state over the existing Happy Agent difference/read APIs.
...
[`master-plans/05-multiple-happy-agents.md`](../../master-plans/05-multiple-happy-agents.md), layered on
the transport and namespace model in
[`master-plans/01-remote-happy-agent.md`](../../master-plans/01-remote-happy-agent.md).

- Happy connects directly only to its local host. Remote Happy Agent traffic continues
  through the host-published peer route.
- A remote Happy Agent remains an ordinary Happy Agent connection with ordinary stores and
  screens.
- Realtime events remain delivery hints. Recovery re-reads durable state.
- UI identity is a contract. Same-Happy Agent notifications and connection transitions
...
- `happyAgentHttpProxy` exposes one capability-scoped loopback surface to the renderer
  and already routes `/nodes/:peerId` through the host.
- `HappyAgentDirectoryStore` owns one `HappyAgentConnectionHandle` and one ordinary
  `HappyAgentSession` per Happy Agent namespace.
- `happyAgentConnectionLoaderCreate` already probes health, heartbeats every two
  seconds, and retries with capped exponential backoff.
- `happy-agent-connect` already reconnects and reconciles core catalog/chat data.
- `HappyAgentWorkspaceStore` already retains drafts, open tabs, files, panel placement,
  and materialized chat stores.
- The terminal driver already records protocol resume offsets/input leases and
  automatically reconnects.
- Happy Agent node, slot, inbox, usage, friends, share, instruction, and security stores
...
### Happy Agent directory lifetime

The directory currently treats only a `connected` peer as a Happy Agent target. When
peer status becomes `connecting` or `unreachable`, it closes and deletes that
Happy Agent. A local runtime transition closes the local Happy Agent and deletes all remotes.
Both paths clear projects and dispose workspace/terminal stores.

### Workspace lifetime

`HappyAgentWorkspaceSurface` currently returns `HappyAgentConnectionStatus` whenever health is
...
One Happy Agent has two independent lifetimes:

1. **Membership** — the local host exists for the desktop lifetime after initial
   bootstrap; a remote Happy Agent exists while it remains in the host's authoritative
   published peer set.
2. **Availability** — the current ability to reach and use that Happy Agent.

The closed availability vocabulary is:

- `connecting`: the Happy Agent has never completed initial hydration in this app
  lifetime.
- `online`: transport is reachable and daemon health is ready.
- `reconnecting`: the Happy Agent was online before and automatic recovery is in
  progress.
...
- `restricted`: a remote machine is reachable but deliberately does not expose
  its Happy Agent API. This remains separate from network failure.
...
| --- | --- |
| Switch Happy Agent, route, project, chat, file tab, panel tab | Enabled |
...
| Message send/steer, slash command, request answer, abort | Disabled with Happy Agent availability reason |
...
| Register local project directory | Disabled while local Happy Agent is unavailable |
...
| Slot text | Visible |
| Slot button with a Happy Agent action | Disabled with reason |
| Browser/webapp already mounted | Retained; Happy Agent-bound navigation/reload disabled |
...
### Phase 2 — Stable multi-Happy Agent directory
...
- [x] Keep each `LiveHappyAgent`, `HappyAgentConnectionHandle`, `HappyAgentSession`, workspace
...
- [x] Subscribe the directory to each Happy Agent's health store and project availability
  into its sidebar entry without replacing the session.
- [x] Preserve `activeHappyAgentId`, order, URL namespace, and router resolution through
...
- [x] Remove the health-driven early return from `HappyAgentWorkspaceSurface`.
...
- [x] Render the notice inside the stable shell/header for the affected Happy Agent.
...
- [x] Add explicit `HappyAgentAvailabilitySnapshot` and availability store/reader types
  in `happy-desktop-state`.
- [x] Derive display status, `online`, and the standard refusal reason once per
  Happy Agent rather than reinterpreting health in every component.
...
- [x] Preserve the grid and terminal component identity while the Happy Agent is down.
...
- [x] Do not enqueue newly typed bytes during a Happy Agent outage.
- [x] Keep protocol auto-reconnect and recovery state; manual retry only
  accelerates it.
- [x] Disable new terminal creation while unavailable.

### Phase 7 — Files, previews, slots, and secondary Happy Agent surfaces
...
- [x] Retain mounted browser/webapp/HTML preview DOM; disable only Happy Agent-bound
  navigation/reload actions.
- [x] Keep slot text visible and pass a connection refusal to Happy Agent-backed slot
  buttons.
...
### Phase 9 — Happy Agent Documents

The Happy Agent-backed Documents surface from
[`master-plans/02-happy-agent-documents.md`](../../master-plans/02-happy-agent-documents.md) does
not exist yet. Local Notes are not a substitute.

- [ ] When Documents is implemented, give every Happy Agent its stable Documents tab and
  collection store.
- [ ] Retain document list, open document, collaborative state, selection, and
  unsaved local changes while that Happy Agent is offline.
...
- [ ] Run the state architecture check. It currently stops on the pre-existing
  ...
  Multiple Happy Agents diff does not touch that file.
...
- [ ] Disconnect/reconnect one remote machine: its section and retained content
  stay while other Happy Agents remain fully usable.
- [ ] Drop the host network while viewing a remote Happy Agent: both host and remote
...
- [ ] Verify compatibility and access-restricted remote Happy Agents still report their
...
dispose or replace any Happy Agent, store, or mounted product surface; availability is
visible per Happy Agent; offline-capable local work continues; online-only actions are
consistently disabled/refused; and automatic recovery reconciles the retained
stores in place.

Happy Agent Documents remains a separate product feature until its master plan is
implemented, but its required Multiple Happy Agents behavior is specified here in advance.
