 ...
 *
 * `/rig-connect/v0/**` is a transparent authenticated Happy Agent bridge. The
 * few routes beside it are genuinely desktop-local services: native Open In and
...
function happyAgentDaemonPath(path: string, query: URLSearchParams): string | undefined {
    ...
    if (daemonPath !== "/v0" && !daemonPath.startsWith("/v0/")) return undefined;
