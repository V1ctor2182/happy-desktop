...
it("holds ProfileCard geometry, typography, presence, and status layout", async () => {
    ...
    const avatar = view.$('[data-testid="pc-full"] [data-happy2-ui="avatar"]');
    ...
    const presence = view.$('[data-testid="pc-full"] [data-happy2-ui="avatar-presence"]');
    ...
    const body = view.$('[data-testid="pc-full"] [data-happy2-ui="profile-card-body"]');
    expect(body.offsets().left, "body left = padding + avatar + gap").toBe(77); /* 17 + 44 + 16 */

    const identity = view.$('[data-testid="pc-full"] [data-happy2-ui="profile-card-identity"]');
    const title = view.$('[data-testid="pc-full"] [data-happy2-ui="profile-card-title"]');
    const status = view.$('[data-testid="pc-full"] [data-happy2-ui="profile-card-status"]');
    ...
    const emojiSlot = view.$(
        '[data-testid="pc-full"] [data-happy2-ui="profile-card-status-emoji"]',
    );
    const emojiGlyph = view.$(
        '[data-testid="pc-full"] [data-happy2-ui="profile-card-status-emoji-glyph"]',
    );
    ...
    const actions = view.$('[data-testid="pc-full"] [data-happy2-ui="profile-card-actions"]');
...
it("holds ProfileCard sizes and content states", async () => {
    ...
    const compactAvatar = view.$('[data-testid="pc-compact"] [data-happy2-ui="avatar"]');
    ...
    const compactBody = view.$('[data-testid="pc-compact"] [data-happy2-ui="profile-card-body"]');
    ...
    const compactName = view.$('[data-testid="pc-compact"] [data-happy2-ui="profile-card-name"]');
    expect(compactName.textMetrics()).toMatchObject({
        font: { lineHeight: 20, size: 16, weight: "700" },
        text: "Grace Hopper",
    });
    /* Presence dot at md is the 8px box. */
    const compactPresence = view.$('[data-testid="pc-compact"] [data-happy2-ui="avatar-presence"]');
    ...
    expect(
        view.container.querySelector(
            '[data-testid="pc-compact"] [data-happy2-ui="profile-card-status-emoji"]',
        ),
    ...
    expect(
        offline.element.querySelector('[data-happy2-ui="avatar-presence"]'),
        "offline shows no presence dot",
    ).toBeNull();
    /* Offline still shows the avatar initials and its title. */
    const offlineInitials = view.$('[data-testid="pc-offline"] [data-happy2-ui="avatar-initials"]');
    await paints(offlineInitials, "offline initials");
    const offlineTitle = view.$('[data-testid="pc-offline"] [data-happy2-ui="profile-card-title"]');
    ...
    expect(
        minimal.element.querySelector('[data-happy2-ui="profile-card-title"]'),
        "minimal has no title",
    ).toBeNull();
    expect(
        minimal.element.querySelector('[data-happy2-ui="profile-card-status"]'),
        "minimal has no status",
    ).toBeNull();
    const minimalName = view.$('[data-testid="pc-minimal"] [data-happy2-ui="profile-card-name"]');
    const minimalUser = view.$(
        '[data-testid="pc-minimal"] [data-happy2-ui="profile-card-username"]',
    );
    expect(
        Math.abs(
    ...
    const imageEl = view.$('[data-testid="pc-image"] [data-happy2-ui="avatar-image"]');
    ...
    expect(
        view.container.querySelector('[data-testid="pc-image"] [data-happy2-ui="avatar-initials"]'),
        "image avatar suppresses initials",
    ).toBeNull();
    /* Image avatar still carries its presence dot and status text. */
    expect(
        view.$('[data-testid="pc-image"] [data-happy2-ui="avatar-presence"]').bounds(),
        "image presence box",
