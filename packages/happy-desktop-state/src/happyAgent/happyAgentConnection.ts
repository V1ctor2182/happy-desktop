import type { HappyAgentDebugLogInput } from "./happyAgentDebugLogStore.js";

/**
 * Minimal Happy Agent daemon health as observed over the injected HTTP probe. This is
 * deliberately a small subset of Happy Agent's protocol health: connection tracking only
 * needs the liveness status and version, never the model catalog or sessions.
 */
export type HappyAgentDaemonHealth =
...
export interface HappyAgentConnectionSnapshot {
...
/**
 * Product-facing availability for one already-known Happy Agent.
 *
 * Connection and daemon health remain the transport facts above. This projection
 * adds the one lifetime fact a surface needs to interpret them: whether this Happy Agent
 ...
 * connectivity flag.
 */
export type HappyAgentAvailabilityState =
    | "connecting"
    | "online"
    | "reconnecting"
    | "offline"
...
export interface HappyAgentAvailabilitySnapshot {
    readonly state: HappyAgentAvailabilityState;
    readonly online: boolean;
    /** Last confirmed state may be shown, but network actions must not run. */
    readonly stale: boolean;
    /** Displayable reason for disabling an action that needs this Happy Agent. */
    readonly refusal?: string;
    /** Short in-context status for a lane, banner, terminal, or sidebar. */
    readonly message: string;
}

/** Outer route state, such as a host's current link to one remote Happy Agent. */
export interface HappyAgentAvailabilityLink {
 ...
 * product surfaces. `materialized` means the Happy Agent has already supplied stores and
 * confirmed state in this app lifetime; losing it is therefore a reconnect, not
 * another startup.
 */
export function happyAgentAvailabilityProject(
    snapshot: HappyAgentConnectionSnapshot,
    materialized: boolean,
    link?: HappyAgentAvailabilityLink,
): HappyAgentAvailabilitySnapshot {
    if (link?.status === "error") {
        const detail = link.message ?? "This Happy Agent reported an error.";
        return {
            message: materialized ? `${detail} Showing the last confirmed state.` : detail,
            online: false,
            refusal: `This Happy Agent is unavailable. ${detail}`,
            stale: materialized,
            state: "error",
        };
    }
    ...
    if (link?.status === "disconnected") {
        const detail = link.message ?? "This Happy Agent is offline.";
        return {
    ...
    if (link?.status === "connecting") {
        const detail = link.message ?? "Connecting to this Happy Agent.";
        return {
            message: materialized ? `${detail} Showing the last confirmed state.` : detail,
            online: false,
            refusal: materialized
                ? "This Happy Agent is reconnecting."
                : "This Happy Agent is still connecting.",
            stale: materialized,
    ...
    if (snapshot.connection === "connected" && snapshot.daemon === "ready")
        return {
            message: snapshot.version
                ? `Happy Agent ${snapshot.version} is online.`
                : "Happy Agent is online.",
            online: true,
            stale: false,
            state: "online",
        };
    ...
    if (snapshot.connection === "connected" && snapshot.daemon === "error") {
        const detail = snapshot.message ?? "The Happy Agent daemon reported an error.";
        return {
            message: detail,
            online: false,
            refusal: `This Happy Agent is unavailable. ${detail}`,
    ...
    // succeeded and none has failed. That is a first connection even on a Happy Agent
        ...
        return {
            message:
                snapshot.connection === "disconnected"
                    ? "Waiting for this Happy Agent to become reachable."
                    : "Connecting to this Happy Agent…",
            online: false,
            refusal: "This Happy Agent is still connecting.",
            stale: false,
            state: "connecting",
        };
    ...
    if (snapshot.connection === "connecting" || snapshot.daemon === "starting")
        return {
            message: "Reconnecting to this Happy Agent. Showing the last confirmed state.",
            online: false,
            refusal: "This Happy Agent is reconnecting.",
            stale: true,
            state: "reconnecting",
        };
    ...
    return {
        message:
            snapshot.attempt > 1
                ? `This Happy Agent is offline. Reconnect attempt ${snapshot.attempt} is automatic.`
                : "This Happy Agent is offline. Reconnecting automatically.",
        online: false,
        refusal: "This Happy Agent is offline. Reconnecting automatically.",
        stale: true,
        state: "offline",
    };
}

export interface HappyAgentConnectionStore {
    get(): HappyAgentConnectionSnapshot;
...
export interface HappyAgentConnectionLoaderOptions {
    /**
     ...
     */
    readonly probe: () => Promise<HappyAgentDaemonHealth>;
    /** Poll interval, in milliseconds, while a probe has succeeded (connected). */
    readonly heartbeatMs?: number;
    /** Consecutive failed probes tolerated before reporting a live Happy Agent as lost. */
    ...
    readonly onDebugEntry?: (entry: HappyAgentDebugLogInput) => void;
}
...
/**
 * Owns the reliability contract for one HTTP connection to a Happy Agent daemon: it
 ...
 */
export function happyAgentConnectionLoaderCreate(
    options: HappyAgentConnectionLoaderOptions,
