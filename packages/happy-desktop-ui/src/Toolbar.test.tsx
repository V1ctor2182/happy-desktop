...
it(
    ...
    async () => {
        ...
        await view.ready();
        const root = (s: string) =>
            view.$(`[data-testid="${s}"] [data-happy-desktop-ui="toolbar"]`);
        const sel = (s: string, name: string) =>
            `[data-testid="${s}"] [data-happy-desktop-ui="toolbar-${name}"]`;
   ...
   14px box, so the toolbar only proves the glyph actually paints. */
        ...
        const fullButtons = view.container.querySelectorAll(
            `${sel("s-full", "trailing")} [data-happy-desktop-ui="button"]`,
        );
        expect(fullButtons.length, "s-full trailing has two buttons").toBe(2);
        const firstButton = view.$(`${sel("s-full", "trailing")} [data-happy-desktop-ui="button"]`);
        ...
        const leadingButton = view.$(
            `${sel("s-leading", "leading")} [data-happy-desktop-ui="button"]`,
        );
