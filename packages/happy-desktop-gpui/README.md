...
```sh
packages/happy-desktop-gpui/scripts/package-macos.sh phase-04-navigation
```
...
The product shell reserves a separate 40 px draggable native titlebar lane. Its reusable Sidebar, Settings, and Command Palette surfaces use the 56 px surface-header contract, `clamp(250px, 30vw, 360px)` navigation width, full-bleed caller-owned scrollbars, and controlled route/focus state. Settings bodies remain Phase 8 work. Chat, file, Create, Inbox, and Social destinations remain typed Phase 4 placeholders for their later phases. The permanent inspector shows an explicit Phase 6 empty state rather than sample rows that could be mistaken for authoritative Git data. Destructive archive and server reorder affordances stay hidden until the shell can pair them with confirmation and mutation state; no local order mutation is presented as server state.
