...
function labelDrift(view: Renderer, testId: string) {
    return inkDrift(
        view,
        `[data-testid="${testId}"]`,
        `[data-testid="${testId}"] [data-happy2-ui="button-content"]`,
    );
}
...
it("holds Button dimensions, typography, and optical label centering for every size and variant", async () => {
    ...
    for (const size of sizes) {
        for (const variant of variants) {
            ...
            const label = view.$(`[data-testid="${id}"] [data-happy2-ui="button-label"]`);
    ...
    const fixedLabel = view.$('[data-testid="button-fixed"] [data-happy2-ui="button-label"]');
...
it("holds Button icon forms and disabled state with optically centered glyphs", async () => {
    ...
    for (const size of sizes) {
        const id = `lead-${size}`;
        const button = view.$(`[data-testid="${id}"]`);
        const icon = view.$(`[data-testid="${id}"] [data-happy2-ui="button-icon"]`);
        const label = view.$(`[data-testid="${id}"] [data-happy2-ui="button-label"]`);
        const buttonBounds = button.bounds();
        ...
        const plainLabel = view.$(`[data-testid="plain-${size}"] [data-happy2-ui="button-label"]`);
