...
function entryShape(entry: ConversationEntry): string {
    ...
    if (entry.kind === "delegation") return "delegation";
...
describe("rigChatStore actions", () => {
    ...
    it("searches a checkout's files for mention candidates", async () => {
        ...
        fake.filesSet(DEFAULT_PROJECT.id as RigGroupId, [
            {
                fileName: "rigChatStore.ts",
                path: "packages/happy-desktop-state/src/rig/rigChatStore.ts",
            },
            { fileName: "rigTypes.ts", path: "packages/happy-desktop-state/src/rig/rigTypes.ts" },
            {
                fileName: "AppRigView.tsx",
