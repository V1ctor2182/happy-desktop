...
it("holds Checkbox geometry, colors, glyph centering, and typography across states", async () => {
    ...
    for (const { id, colors } of boxCases) {
        const box = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-box"]`);
    ...
    for (const id of ["cb-unchecked", "cb-checked", "cb-indeterminate"]) {
        ...
        const box = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-box"]`);
        const label = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-label"]`);
    ...
    const label = view.$('[data-testid="cb-checked"] [data-happy-desktop-ui="checkbox-label"]');
    ...
    for (const id of ["cb-checked", "box-checked"]) {
        const box = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-box"]`);
        const glyph = view.$(
            `[data-testid="${id}"] [data-happy-desktop-ui="checkbox-box"] [data-happy-desktop-ui="icon"]`,
        );
        const boxBounds = box.bounds();
        const glyphBounds = glyph.bounds();
    ...
    for (const id of ["cb-indeterminate", "box-indeterminate"]) {
        const dash = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-mark"]`);
        expect(dash.bounds().width, `${id} dash width`).toBe(8);
        expect(dash.bounds().height, `${id} dash height`).toBe(2);
        const drift = await glyphDrift(
            view,
            `[data-testid="${id}"] [data-happy-desktop-ui="checkbox-box"]`,
            `[data-testid="${id}"] [data-happy-desktop-ui="checkbox-mark"]`,
        );
    ...
    for (const id of ["cb-unchecked", "box-unchecked"]) {
        const box = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-box"]`);
        expect(
            box.element.querySelector('[data-happy-desktop-ui="icon"]'),
            `${id} no check`,
        ).toBeNull();
        expect(
            box.element.querySelector('[data-happy-desktop-ui="checkbox-mark"]'),
            `${id} no dash`,
        ).toBeNull();
    ...
    for (const id of ["box-unchecked", "box-checked", "box-indeterminate"]) {
        const root = view.$(`[data-testid="${id}"]`);
        expect(
            root.element.querySelector('[data-happy-desktop-ui="checkbox-label"]'),
            `${id} label absent`,
        ).toBeNull();
    ...
    expect(
        (
            view.$('[data-testid="cb-indeterminate"] [data-happy-desktop-ui="checkbox-control"]')
                .element as HTMLInputElement
        ).indeterminate,
    ).toBe(true);
    expect(
        (
            view.$('[data-testid="cb-checked"] [data-happy-desktop-ui="checkbox-control"]')
                .element as HTMLInputElement
        ).indeterminate,
    ).toBe(false);
    ...
    for (const id of ["cb-disabled", "cb-disabled-checked"]) {
        ...
        expect(
            (
                view.$(`[data-testid="${id}"] [data-happy-desktop-ui="checkbox-control"]`)
                    .element as HTMLInputElement
            ).disabled,
        ).toBe(true);
    }
    ...
    expect(
        (
            await view
                .$(
                    '[data-testid="cb-disabled-checked"] [data-happy-desktop-ui="checkbox-box"] [data-happy-desktop-ui="icon"]',
                )
                .visibleMetrics()
        ).pixelCount,
    ).toBeGreaterThan(0);
...
it("holds Checkbox focus ring and reports changes through onChange", async () => {
    ...
    const box = view.$('[data-testid="focus-cb"] [data-happy-desktop-ui="checkbox-box"]');
    const input = view.$('[data-testid="focus-cb"] [data-happy-desktop-ui="checkbox-control"]')
        .element as HTMLInputElement;
