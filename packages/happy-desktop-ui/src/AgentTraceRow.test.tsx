...
async function dotDrift(view: View, rowSelector: string) {
    ...
    const dot = view.$(`${rowSelector} [data-happy-desktop-ui="agent-trace-row-dot"]`);
...
it("holds AgentTraceRow geometry, states, icon mapping, and click behavior", async () => {
    ...
    view.render(
        () => (
            ...
            >
                ...
                <AgentTraceRow
                    data-testid="tr-tool"
                    detail="pnpm --dir packages/happy-desktop-ui typecheck"
                    entryCount={4}
                    kind="tool"
                    status="running"
                    title="Bash"
    ...
    const dotOf = (testid: string) =>
        view.$(`[data-testid="${testid}"] [data-happy-desktop-ui="agent-trace-row-dot"]`);
    ...
    const iconName = (testid: string) =>
        view
            .$(`[data-testid="${testid}"] [data-happy-desktop-ui="icon"]`)
    ...
    expect(
        view.container.querySelector('[data-testid="tr-complete"] [data-happy-desktop-ui="icon"]'),
    ).toBeNull();

    /* ---- Running content: title, mono detail, mono count ------------------ */

    const title = view.$(
        '[data-testid="tr-running"] [data-happy-desktop-ui="agent-trace-row-title"]',
    );
    ...
    const detail = view.$(
        '[data-testid="tr-running"] [data-happy-desktop-ui="agent-trace-row-detail"]',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="tr-running"] [data-happy-desktop-ui="agent-trace-row-count"]',
        ),
    ).toBeNull();
    ...
    const icon = view.$(
        '[data-testid="tr-running"] [data-happy-desktop-ui="agent-trace-row-icon"]',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="tr-subagent"] [data-happy-desktop-ui="agent-trace-row-detail"]',
        ),
    ).toBeNull();
    expect(
        view.container.querySelector(
            '[data-testid="tr-subagent"] [data-happy-desktop-ui="agent-trace-row-count"]',
        ),
    ).toBeNull();
    ...
    const completeTitle = view.$(
        '[data-testid="tr-complete"] [data-happy-desktop-ui="agent-trace-row-title"]',
    );
    expect(completeTitle.element.textContent).toBe("View traces");
    expect(completeTitle.computedStyle("color")).toBe("rgb(43, 172, 204)");
    expect(
        view.$('[data-testid="tr-complete"] [data-happy-desktop-ui="agent-trace-row-count"]')
            .element.textContent,
    ).toBe("12 steps");
    expect(
        view.$('[data-testid="tr-failed"] [data-happy-desktop-ui="agent-trace-row-count"]').element
            .textContent,
    ).toBe("3 steps");
    ...
    const narrowDetail = view.$(
        '[data-testid="tr-narrow"] [data-happy-desktop-ui="agent-trace-row-detail"]',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="tr-narrow"] [data-happy-desktop-ui="agent-trace-row-count"]',
        ),
    ).toBeNull();
