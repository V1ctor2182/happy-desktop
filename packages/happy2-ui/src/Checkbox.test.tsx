...
it("holds Checkbox geometry, colors, glyph centering, and typography across states", async () => {
    ...
    for (const id of ["cb-unchecked", "cb-checked", "cb-indeterminate"]) {
        ...
        const box = view.$(`[data-testid="${id}"] [data-happy2-ui="checkbox-box"]`);
        const label = view.$(`[data-testid="${id}"] [data-happy2-ui="checkbox-label"]`);
    ...
    for (const id of ["cb-indeterminate", "box-indeterminate"]) {
        const dash = view.$(`[data-testid="${id}"] [data-happy2-ui="checkbox-mark"]`);
        ...
        const drift = await glyphDrift(
            ...
            `[data-testid="${id}"] [data-happy2-ui="checkbox-mark"]`,
        );
    ...
    for (const id of ["box-unchecked", "box-checked", "box-indeterminate"]) {
        const root = view.$(`[data-testid="${id}"]`);
        expect(
            root.element.querySelector('[data-happy2-ui="checkbox-label"]'),
            `${id} label absent`,
        ).toBeNull();
    ...
    expect(
        (
            view.$('[data-testid="cb-indeterminate"] [data-happy2-ui="checkbox-control"]')
                .element as HTMLInputElement
        ).indeterminate,
    ).toBe(true);
    expect(
        (
            view.$('[data-testid="cb-checked"] [data-happy2-ui="checkbox-control"]')
                .element as HTMLInputElement
        ).indeterminate,
    ).toBe(false);
    ...
    for (const id of ["cb-disabled", "cb-disabled-checked"]) {
        ...
        expect(
            (
                view.$(`[data-testid="${id}"] [data-happy2-ui="checkbox-control"]`)
                    .element as HTMLInputElement
            ).disabled,
        ).toBe(true);
    }
...
it("holds Checkbox focus ring and reports changes through onChange", async () => {
    ...
    const box = view.$('[data-testid="focus-cb"] [data-happy2-ui="checkbox-box"]');
    const input = view.$('[data-testid="focus-cb"] [data-happy2-ui="checkbox-control"]')
        .element as HTMLInputElement;
