...
```tsx
import { Box, Button } from "happy-desktop-ui";
...
```tsx
import type { ChatStore } from "happy-desktop-state";
import { Message, MessageList, StoreSurface } from "happy-desktop-ui";
...
the real deterministic fixture builders from `happy-desktop-state/testing`. When the `store` prop changes,
...
import { Box, Button } from "happy-desktop-ui";
import { createRenderer } from "happy-desktop-ui/testing";
...
rewrite tracked PNGs. Run `pnpm --filter happy-desktop-ui test:artifacts` when deliberately refreshing
