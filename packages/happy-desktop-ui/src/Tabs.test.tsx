...
it("holds Tabs arity, active sweep, and count badges", async () => {
    ...
    for (const [selector, count, activeId] of [
        ...
        expect(
            bar.querySelectorAll('[data-happy-desktop-ui="tab-underline"]').length,
            `${activeId} one underline`,
        ).toBe(1);
        expect(
            bar
                .querySelector(
                    `[data-tab-id="${activeId}"] [data-happy-desktop-ui="tab-underline"]`,
                )
    ...
    for (const active of ["all", "mentions", "reactions"] as const) {
        ...
        const underline = view.$(
            `[data-tab-id="${active}"][aria-selected="true"] [data-happy-desktop-ui="tab-underline"]`,
        );
