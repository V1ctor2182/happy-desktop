...
it("holds TerminalPanel geometry, full-bleed screen, and lifecycle controls", async () => {
    ...
    const rows = view.$('[data-testid="connected"] [data-happy-desktop-ui="terminal-rows"]');
    ...
    expect(
        view.container.querySelector(
            '[data-testid="exited"] [data-happy-desktop-ui="terminal-screen"]',
        ),
    ).toBeNull();
    expect(
        view.container.querySelector(
...
it("forwards focused printable and control-key terminal input", async () => {
    ...
    expect(
        view.$('[data-happy-desktop-ui="terminal-screen"]').computedStyles(["box-shadow"])[
            "box-shadow"
        ],
    ).toBe("none");
    const caret = view
...
it("does not steal focus from the chat composer when its resize callback changes", async () => {
    ...
    await userEvent.click(view.$('[data-happy-desktop-ui="terminal-screen"]').element);
    expect(document.activeElement).toBe(terminal);
});
