...
/** A `happy2-state` handle whose owner must release it. */
type Disposable = { [Symbol.dispose](): void };

/**
 * Leases one disposable `happy2-state` handle for as long as a surface needs it,
 * re-leasing when `key` changes and releasing on unmount.
 *
 * `happy2-state` hands out per-entity handles that the holder must dispose, and a
