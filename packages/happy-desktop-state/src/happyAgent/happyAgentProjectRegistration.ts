...
/**
 * Turns Happy Agent's refusal to register a folder into something the reader can act on.
 *
 * Happy Agent decides what may become a project and says why in a closed set of codes;
 ...
 * it.
 *
 * A code this build does not know is not swallowed: Happy Agent's own message is shown,
 ...
 */
export function happyAgentProjectAddError(error: unknown, path: string): UserError {
    ...
    if (error instanceof ProjectRegistrationProtocolError) {
        return new UserError(
            error.code === "request_failed"
                ? `“${folder}” could not be added: this machine’s Happy Agent did not answer. Try again.`
                : `“${folder}” could not be added: this machine’s Happy Agent gave an answer Happy could not read.`,
            undefined,
            error,
        );
    }
 ...
 * which leaves Happy Agent's own wording in place.
 */
function registrationMessage(
    ...
    switch (code) {
        ...
        // rather than the reader choosing wrongly, and Happy Agent names the exact rule
