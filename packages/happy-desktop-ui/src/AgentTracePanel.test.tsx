...
const entries: readonly AgentTracePanelEntry[] = [
    ...
    },
    {
        ...
        kind: "tool",
        title: "Read",
        detail: "packages/happy-desktop-state/src/index.ts",
        status: "complete",
    ...
    },
    {
        ...
        kind: "terminal",
        title: "Terminal",
        detail: "pnpm --dir packages/happy-desktop-state typecheck",
        status: "failed",
...
it("holds AgentTracePanel header, scrollport contract, entry rows, and timestamps", async () => {
    ...
    const header = view.$('[data-testid="tp-running"] [data-happy-desktop-ui="toolbar"]');
    expect(header.height()).toBe(56);
    expect(header.width()).toBe(288);
    expect(
        view.$('[data-testid="tp-running"] [data-happy-desktop-ui="toolbar-title"]').element
            .textContent,
    ).toBe("Codey");
    expect(
        view.$('[data-testid="tp-running"] [data-happy-desktop-ui="toolbar-subtitle"]').element
            .textContent,
    ).toBe("8 steps");
    const badge = view.$('[data-testid="tp-running"] [data-happy-desktop-ui="badge"]');
    expect(badge.element.getAttribute("data-variant")).toBe("accent");
    expect(
        view.$('[data-testid="tp-running"] [data-happy-desktop-ui="badge-label"]').element
            .textContent,
    ).toBe("RUNNING");
    const close = view.$('[data-testid="tp-running"] [data-happy-desktop-ui="button"]');
    ...
    const body = view.$(
        '[data-testid="tp-running"] [data-happy-desktop-ui="agent-trace-panel-body"]',
    );
    ...
    const wrapper = view.$(
        '[data-testid="tp-running"] [data-happy-desktop-ui="agent-trace-panel-entries"]',
    );
    expect(
    ...
    const rows = Array.from(
        panel.element.querySelectorAll('[data-happy-desktop-ui="agent-trace-panel-entry"]'),
    ) as HTMLElement[];
    ...
    const iconOf = (row: HTMLElement) =>
        row.querySelector('[data-happy-desktop-ui="icon"]')?.getAttribute("data-name");
    ...
    const rowDot = (index: number) =>
        view.$(
            `[data-testid="tp-running"] [data-happy-desktop-ui="agent-trace-panel-entries"] > :nth-child(${index + 1}) [data-happy-desktop-ui="agent-trace-panel-entry-dot"]`,
        );
    ...
    const drift = await dotDrift(
        view,
        '[data-testid="tp-running"] [data-happy-desktop-ui="agent-trace-panel-entries"] > :nth-child(1)',
    );
    ...
    const title = view.$(`${second} [data-happy-desktop-ui="agent-trace-panel-entry-title"]`);
    ...
    const detail = view.$(`${second} [data-happy-desktop-ui="agent-trace-panel-entry-detail"]`);
    ...
    const time = view.$(`${second} [data-happy-desktop-ui="agent-trace-panel-entry-time"]`);
    ...
    const timeText = (index: number) =>
        view.$(
            `[data-testid="tp-running"] [data-happy-desktop-ui="agent-trace-panel-entries"] > :nth-child(${index + 1}) [data-happy-desktop-ui="agent-trace-panel-entry-time"]`,
        ).element.textContent;
    ...
    expect(
        (
            await view
                .$(`${noDetail} [data-happy-desktop-ui="agent-trace-panel-entry-title"]`)
                .visibleMetrics()
        ).pixelCount,
    ).toBeGreaterThan(0);
...
it("scrolls overflowing traces edge to edge and centers loading, error, and empty states", async () => {
    ...
    const first = view.$(
        '[data-testid="tp-scroll"] [data-happy-desktop-ui="agent-trace-panel-entries"] > :nth-child(1)',
    );
    ...
    expect(
        (
            await view
                .$(
                    '[data-testid="tp-scroll"] [data-happy-desktop-ui="agent-trace-panel-entries"] > :nth-child(1) [data-happy-desktop-ui="agent-trace-panel-entry-title"]',
                )
                .visibleMetrics()
        ).pixelCount,
    ).toBeGreaterThan(0);
    ...
    expect(
        (
            await view
                .$(
                    '[data-testid="tp-scroll"] [data-happy-desktop-ui="agent-trace-panel-entries"] > :nth-child(20) [data-happy-desktop-ui="agent-trace-panel-entry-title"]',
                )
                .visibleMetrics()
        ).pixelCount,
    ).toBeGreaterThan(0);
    ...
    const stateOf = (testid: string) =>
        view.$(`[data-testid="${testid}"] [data-happy-desktop-ui="agent-trace-panel-state"]`);
    ...
    const loadingBody = view.$(
        '[data-testid="tp-loading"] [data-happy-desktop-ui="agent-trace-panel-body"]',
    );
    ...
    const badgeOf = (testid: string) => {
        const badge = view.$(`[data-testid="${testid}"] [data-happy-desktop-ui="badge"]`);
        return {
            label: view.$(`[data-testid="${testid}"] [data-happy-desktop-ui="badge-label"]`).element
                .textContent,
    ...
    expect(
        view
            .$('[data-testid="tp-error"] [data-happy-desktop-ui="button"]')
            .element.getAttribute("aria-label"),
    ).toBe("Dismiss trace");
    /* No close button when the host passes no onClose. */
    expect(
        view.container.querySelector('[data-testid="tp-empty"] [data-happy-desktop-ui="button"]'),
    ).toBeNull();

    /* Zero-count subtitle uses the singular-aware grammar. */
    expect(
        view.$('[data-testid="tp-empty"] [data-happy-desktop-ui="toolbar-subtitle"]').element
            .textContent,
    ).toBe("0 steps");
