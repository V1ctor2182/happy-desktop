 ...
 *
 * It exists because appearance is product state, not React state: both the cloud
 * workspace and the local Rig workspace render one `ThemeScope` from it, so the
 * two modes are two renderings of one mechanism rather than two theme
 * implementations. Keeping it here also keeps the selection out of the
