...
it("shows the probe loader while the transport is still connecting", async () => {
    ...
    expect(
        view
            .$('[data-happy-desktop-ui="rig-connection-status"]')
            .element.getAttribute("data-state"),
    ).toBe("connecting");
    expect(
        view.$('[data-happy-desktop-ui="rig-connection-status-label"]').element.textContent,
    ).toBe("Connecting to Rig");
    expect(
        view.$('[data-happy-desktop-ui="rig-connection-status-progress"]').element.textContent,
    ).toBe("Checking the local service…");
    expect(view.container.querySelector('[data-happy-desktop-ui="spinner"]')).not.toBeNull();
    expect(
        view.container.querySelector('[data-happy-desktop-ui="rig-connection-status-body"] button'),
    ).toBeNull();
...
it("counts reconnect attempts and retries on demand while disconnected", async () => {
    ...
    expect(
        view.$('[data-happy-desktop-ui="rig-connection-status-label"]').element.textContent,
    ).toBe("Reconnecting to Rig");
    expect(
        view.$('[data-happy-desktop-ui="rig-connection-status-progress"]').element.textContent,
    ).toBe("Waiting for the local service · attempt 3");
    const retry = view.$('[data-happy-desktop-ui="rig-connection-status-body"] button')
...
it("offers an explicit retry control when a reachable daemon reports an error", async () => {
    ...
    expect(
        view.$('[data-happy-desktop-ui="rig-connection-status-label"]').element.textContent,
    ).toBe("Rig needs attention");
    expect(
        view.$('[data-happy-desktop-ui="rig-connection-status-progress"]').element.textContent,
    ).toBe("No provider is authenticated.");
    const button = view.$('[data-happy-desktop-ui="rig-connection-status-body"] button')
        .element as HTMLButtonElement;
