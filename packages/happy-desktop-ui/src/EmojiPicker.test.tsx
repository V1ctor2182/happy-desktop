...
async function emojiInk(view: Renderer, cellSelector: string, inner: "glyph" | "image") {
    const art = view.$(`${cellSelector} [data-happy-desktop-ui="emoji-picker-art"]`);
    const el = view.$(`${cellSelector} [data-happy-desktop-ui="emoji-picker-${inner}"]`);
...
it("holds EmojiPicker card, search, fixed grid slots, and per-emoji centering", async () => {
    ...
    const control = view.$('[data-testid="picker"] [data-happy-desktop-ui="text-field-control"]');
    ...
    const input = view.$('[data-testid="picker"] [data-happy-desktop-ui="text-field-input"]')
        .element as HTMLInputElement;
    expect(input.type).toBe("search");
    expect(input.value).toBe("");
    expect(input.placeholder).toBe("Search emoji");
    const searchIcon = view.$(
        '[data-testid="picker"] [data-happy-desktop-ui="text-field-icon"] [data-happy-desktop-ui="icon"]',
    );
    ...
    const grid = view.$('[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"]');
    ...
    emoji.forEach((item, index) => {
        const cell = view.$(
            `[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"] [data-emoji-id="${item.id}"]`,
        );
        const b = cell.bounds();
    ...
    for (const item of emoji) {
        const cell = `[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"] [data-emoji-id="${item.id}"]`;
        const art = view.$(`${cell} [data-happy-desktop-ui="emoji-picker-art"]`);
    ...
    for (const id of ["relay", "mint"]) {
        const image = view.$(
            `[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"] [data-emoji-id="${id}"] [data-happy-desktop-ui="emoji-picker-image"]`,
        );
    ...
    for (const id of inkSubset) {
        ...
        const cell = `[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"] [data-emoji-id="${id}"]`;
    ...
    (
        view.$(
            '[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"] [data-emoji-id="thumbsup"]',
        ).element as HTMLButtonElement
    ).click();
    (
        view.$(
            '[data-testid="picker"] [data-happy-desktop-ui="emoji-picker-grid"] [data-emoji-id="relay"]',
        ).element as HTMLButtonElement
    ).click();
...
it("holds EmojiPicker recent sections, searching, empty state, and query events", async () => {
    ...
    const recentLabel = view.$(
        '[data-testid="recent"] [data-happy-desktop-ui="emoji-picker-recent-label"]',
    );
    ...
    const allLabel = view.$(
        '[data-testid="recent"] [data-happy-desktop-ui="emoji-picker-all-label"]',
    );
    expect(allLabel.element.textContent).toBe("All emoji");

    const recentGrid = view.$(
        '[data-testid="recent"] [data-happy-desktop-ui="emoji-picker-recent-grid"]',
    );
    const recentCells = recentGrid.element.querySelectorAll(
    ...
    ["thumbsup", "tada", "rocket", "fire", "heart", "relay"].forEach((id, index) => {
        const cell = view.$(
            `[data-testid="recent"] [data-happy-desktop-ui="emoji-picker-recent-grid"] [data-emoji-id="${id}"]`,
        );
    ...
    const allGrid = view.$('[data-testid="recent"] [data-happy-desktop-ui="emoji-picker-grid"]');
    expect(
        allGrid.element.querySelectorAll('[data-happy-desktop-ui="emoji-picker-cell"]').length,
        "all cell count",
    ).toBe(24);
    ...
    (
        view.$(
            '[data-testid="recent"] [data-happy-desktop-ui="emoji-picker-recent-grid"] [data-emoji-id="fire"]',
        ).element as HTMLButtonElement
    ).click();
    ...
    expect(
        searching.element.querySelector('[data-happy-desktop-ui="emoji-picker-recent-section"]'),
        "recent hidden while searching",
    ).toBeNull();
    expect(
        searching.element.querySelector('[data-happy-desktop-ui="emoji-picker-all-label"]'),
        "no all-label without a recent row",
    ).toBeNull();
    const searchInput = view.$(
        '[data-testid="searching"] [data-happy-desktop-ui="text-field-input"]',
    ).element as HTMLInputElement;
    expect(searchInput.value, "query drives the field").toBe("fire");
    expect(
        view
            .$('[data-testid="searching"] [data-happy-desktop-ui="emoji-picker-grid"]')
            .element.querySelectorAll('[data-happy-desktop-ui="emoji-picker-cell"]').length,
    ...
    expect(
        empty.element.querySelector('[data-happy-desktop-ui="emoji-picker-grid"]'),
        "no grid when empty",
    ).toBeNull();
    const emptyMsg = view.$('[data-testid="empty"] [data-happy-desktop-ui="emoji-picker-empty"]');
