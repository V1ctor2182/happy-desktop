...
export type RigProjectCloneDialogProps = {
    /**
     * The GitHub repository to clone. The caller owns this draft so it remains
     * intact while its Rig publishes project changes around the dialog.
     */
    repository: string;
    /** True while the Rig is creating the checkout. */
    submitting?: boolean;
    /** Why the last clone attempt failed, in the Rig's own words. */
    error?: string;
    /** Why this Rig cannot currently accept a clone request. */
...
/**
 * C-256 RigProjectCloneDialog — clone a GitHub project onto one Rig.
 *
 ...
 * the checkout lands is a decision for the addressed Rig, not another local
 ...
 */
export function RigProjectCloneDialog(props: RigProjectCloneDialogProps) {
    ...
    return (
        ...
        >
            ...
            >
                <div
                    className="happy2-rig-project-clone-dialog"
                    data-happy-desktop-ui="rig-project-clone-dialog"
                >
                    {props.submitDisabledReason ? (
                        <Banner tone="neutral" title="Rig unavailable">
                            {props.submitDisabledReason}
                        </Banner>
                    ) : null}
                    {props.error ? (
                    ...
                    />
                    <p
                        className="happy2-rig-project-clone-dialog__hint"
                        data-happy-desktop-ui="rig-project-clone-dialog-hint"
                    >
                        Clone a repository this Rig can access.
                    </p>
                </div>
            </Modal>
        </ModalOverlay>
