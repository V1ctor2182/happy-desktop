...
function sessionStatusProject(session: SessionState): RigSessionStatus {
    if (session.archived) return "archived";
    return session.status === "failed" ? "error" : session.status;
}

function sessionProject(session: SessionState): RigSession {
    const projectId = (session.projectId ??
        session.workspaceId ??
        session.sessionId) as RigProjectId;
    const scope =
            ...
            : {
                  ...
                  projectId:
                      session.scope.kind === "project"
                          ? (session.scope.projectId as RigProjectId)
                          : projectId,
              };
    return {
        id: session.sessionId as RigSessionId,
        scope,
        ...
        models: [],
        ...(session.effort === undefined ? {} : { effort: session.effort as RigThinkingLevel }),
        ...(session.serviceTier === undefined
            ? {}
            : { serviceTier: session.serviceTier as RigServiceTier }),
