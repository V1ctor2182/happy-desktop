...
`happy-desktop-state` is Happy (2)'s framework-independent, memory-only product-state boundary. A host may
...
`happy-desktop-state/testing` exports a programmable fake server with request history, deferred handlers,
