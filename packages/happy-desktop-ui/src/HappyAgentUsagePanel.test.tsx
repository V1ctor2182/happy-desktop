...
import type { HappyAgentSessionUsage } from "happy-desktop-state";
import "./theme.css";
import "./styles/happy-agent-usage.css";
import { HappyAgentUsagePanel } from "./HappyAgentUsagePanel";
...
it("renders per-model token/cost rows, totals, context, and quota windows", async () => {
    ...
    view.render(() => <HappyAgentUsagePanel data-testid="u" usage={usage} />, {
        width: 620,
        height: 520,
        padding: 16,
    });
    ...
    expect(
        view.$('[data-testid="u"] [data-happy-desktop-ui="happy-agent-usage-totals"]').element
            .textContent,
    ).toContain("307,500");

    // Context line reports approximate occupancy.
    expect(
        view.$('[data-testid="u"] [data-happy-desktop-ui="happy-agent-usage-context"]').element
            .textContent,
    ).toContain("approximate");

    // Two quota windows, the 93% one flagged full.
    const windows = view.container.querySelectorAll(
        '[data-testid="u"] [data-happy-desktop-ui="happy-agent-usage-quota-window"]',
    );
...
it("shows loading and error states without a snapshot", async () => {
    ...
    view.render(
        () => (
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", width: "560px" }}>
                <HappyAgentUsagePanel data-testid="loading" loading />
                <HappyAgentUsagePanel data-testid="error" error="Usage unavailable." />
            </div>
        ),
    ...
    expect(
        view.container.querySelector(
            '[data-testid="loading"] [data-happy-desktop-ui="happy-agent-usage-empty"]',
        ),
    ).not.toBeNull();
    expect(
        view.$('[data-testid="error"] [data-happy-desktop-ui="happy-agent-usage-error"]').element
            .textContent,
