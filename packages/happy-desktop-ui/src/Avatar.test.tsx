    ...
    async () => {
        ...
        for (const { size, dimension, fontSize, agentRadius } of SIZE_CASES) {
            for (const [index, content] of CONTENT_CASES.entries()) {
                ...
                const initials = view.$(
                    `[data-testid="${id}"] [data-happy-desktop-ui="avatar-initials"]`,
                );
...
it("anchors the presence dot at every size", { timeout: 90_000 }, async () => {
    ...
    for (const { id, dimension, dot, x } of presenceCases) {
        const presence = view.$(`[data-testid="${id}"] [data-happy-desktop-ui="avatar-presence"]`);
...
it("renders the image variant, tones, defaults, and keeps size in tight flex rows", async () => {
    ...
    const image = view.$('[data-testid="avatar-image"] [data-happy-desktop-ui="avatar-image"]');
    ...
    expect(
        view.$('[data-testid="avatar-image"] [data-happy-desktop-ui="avatar-presence"]').bounds(),
    ).toEqual({ x: 41, y: 41, width: 8, height: 8 });

    const agentImage = view.$(
        '[data-testid="avatar-image-agent"] [data-happy-desktop-ui="avatar-image"]',
    );
    ...
    for (const [tone, color] of Object.entries(TONE_COLORS)) {
        ...
        const chipInitials = view.$(
            `[data-testid="tone-${tone}"] [data-happy-desktop-ui="avatar-initials"]`,
        );
