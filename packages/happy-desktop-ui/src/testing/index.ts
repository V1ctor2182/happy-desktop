...
} from "happy-desktop-gym/playwright";
export { RenderedElement } from "happy-desktop-gym/playwright";
export type {
    Bounds,
    EdgeOffsets,
    RenderedTextMetrics,
...
function identifier(element: Element) {
    const part = element.getAttribute("data-happy-desktop-ui");
    const testId = element.getAttribute("data-testid");
    return [
        element.tagName.toLowerCase(),
        part && `[data-happy-desktop-ui="${part}"]`,
...
/**
 * Audits every rounded happy-desktop-ui part below `root`. When a descendant corner
...
export function assertParallelRoundedCorners(root: ParentNode) {
    const elements = Array.from(root.querySelectorAll("[data-happy-desktop-ui]"));
