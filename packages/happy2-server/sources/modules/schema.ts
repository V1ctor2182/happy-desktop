...
export const projects = sqliteTable(
    ...
    (table) => [
        uniqueIndex("projects_one_default_idx")
            .on(table.isDefault)
            .where(sql`${table.isDefault} = 1`),
        index("projects_sync_sequence_idx").on(table.syncSequence),
        check("projects_is_default_check", sql`${table.isDefault} IN (0, 1)`),
    ],
);
...
export const chats = sqliteTable(
    ...
    (table) => [index("chats_project_id_idx").on(table.projectId)],
);
