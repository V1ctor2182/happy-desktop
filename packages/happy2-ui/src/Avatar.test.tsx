...
it("anchors the presence dot at every size", { timeout: 90_000 }, async () => {
    ...
    for (const { id, dimension, dot, x } of presenceCases) {
        const presence = view.$(`[data-testid="${id}"] [data-happy2-ui="avatar-presence"]`);
...
it("renders the image variant, tones, defaults, and keeps size in tight flex rows", async () => {
    ...
    const image = view.$('[data-testid="avatar-image"] [data-happy2-ui="avatar-image"]');
    ...
    expect(
        view.$('[data-testid="avatar-image"] [data-happy2-ui="avatar-presence"]').bounds(),
    ).toEqual({ x: 41, y: 41, width: 8, height: 8 });

    const agentImage = view.$('[data-testid="avatar-image-agent"] [data-happy2-ui="avatar-image"]');
