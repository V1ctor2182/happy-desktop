...
it("flips the audience on Shift+Tab, keeps focus, draft, and textarea identity", async () => {
    ...
    await view.ready();
    const textarea = view.$(
        '[data-testid="composer-shift-tab"] [data-happy-desktop-ui="composer-textarea"]',
    ).element as HTMLTextAreaElement;
    const toggleValue = () =>
        view.container
            .querySelector(
                '[data-testid="composer-shift-tab"] [data-happy-desktop-ui="audience-toggle"]',
            )
    ...
    expect(
        view.container.querySelector(
            '[data-testid="composer-shift-tab"] [data-happy-desktop-ui="composer-agents"]',
        ),
    ).toBeNull();
...
it("keeps plain Tab mention selection working and lets Shift+Tab through the picker", async () => {
    ...
    await view.ready();
    const textarea = view.$(
        '[data-testid="composer-mention-tab"] [data-happy-desktop-ui="composer-textarea"]',
    ).element as HTMLTextAreaElement;
    const popover = () =>
        view.container.querySelector(
            '[data-testid="composer-mention-tab"] [data-happy-desktop-ui="composer-popover"]',
        );
    const toggleValue = () =>
        view.container
            .querySelector(
                '[data-testid="composer-mention-tab"] [data-happy-desktop-ui="audience-toggle"]',
            )
            ?.getAttribute("data-value");
    await userEvent.click(textarea);
...
it("does not intercept Shift+Tab when the composer has no audience routing", async () => {
    ...
    await view.ready();
    const textarea = view.$(
        '[data-testid="composer-no-audience"] [data-happy-desktop-ui="composer-textarea"]',
    ).element as HTMLTextAreaElement;
    await userEvent.click(textarea);
    await userEvent.keyboard("{Shift>}{Tab}{/Shift}");
...
it("reports textarea focus and blur without replacing the controlled DOM node", async () => {
    ...
    await view.ready();
    const textarea = view.$(
        '[data-testid="composer-focus"] [data-happy-desktop-ui="composer-textarea"]',
    ).element as HTMLTextAreaElement;
    await userEvent.click(textarea);
    await userEvent.keyboard("local text");
    expect(document.activeElement).toBe(textarea);
    expect(
        view.$('[data-testid="composer-focus"] [data-happy-desktop-ui="composer-textarea"]')
            .element,
    ).toBe(textarea);
...
it("keeps the audience and capability actions inside a panel-constrained composer", async () => {
    ...
    const surface = view.$(
        '[data-testid="composer-compact"] [data-happy-desktop-ui="composer-surface"]',
    );
    const toggle = view.$(
        '[data-testid="composer-compact"] [data-happy-desktop-ui="audience-toggle"]',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="composer-compact"] [data-happy-desktop-ui="composer-hint"]',
        ),
    ).toBeNull();
...
it("marks Agents mode with a quiet accent frame instead of a chip row", async () => {
    ...
    expect(view.container.querySelector('[data-happy-desktop-ui="composer-agents"]')).toBeNull();
    expect(
        view.container.querySelector('[data-happy-desktop-ui="composer-agent-chip"]'),
    ).toBeNull();
...
it("keeps only one mention or emoji picker open", async () => {
    ...
    const mentionPopover = () =>
        view.container.querySelector(
            '[data-testid="composer-exclusive-pickers"] [data-happy-desktop-ui="composer-popover"]',
        );
    const emojiPopover = () =>
        view.container.querySelector(
            '[data-testid="composer-exclusive-pickers"] [data-happy-desktop-ui="composer-emoji-popover"]',
        );
    view.container
