...
it("renders the automated marker structure and accessible role", { timeout: 90000 }, async () => {
    ...
    await view.ready();
    const tag = view.$('[data-happy-desktop-ui="automated-tag"]');
    ...
    const icon = view.$('[data-happy-desktop-ui="automated-tag-icon"]');
    const label = view.$('[data-happy-desktop-ui="automated-tag-label"]');
...
it(
    ...
    async () => {
        ...
        await view.ready();
        const iconInk = await inkCenter(
            "icon",
            view.$('[data-happy-desktop-ui="automated-tag-icon"]'),
        );
        const labelInk = await inkCenter(
            "label",
            view.$('[data-happy-desktop-ui="automated-tag-label"]'),
        );
