...
export function FilesPage(props: FilesPageProps) {
    return (
        <StoreSurface store={props.store}>
            {(snapshot) => <FilesPageContent {...props} snapshot={snapshot} />}
        </StoreSurface>
    );
}

function FilesPageContent(props: FilesPageProps & { snapshot: FilesSnapshot }) {
    ...
    return source.length > 0 ? (
        ...
        <EmptyState
            ...
            icon="files"
            title={snapshot.status.type === "loading" ? "Loading files…" : "No shared files"}
        />
    ) : (
    ...
    return {
        duration:
            file.durationMs === undefined
                ? undefined
                : `${Math.floor(file.durationMs / 60000)}:${Math.round(
                      (file.durationMs % 60000) / 1000,
                  )
                      .toString()
                      .padStart(2, "0")}`,
        id: file.id,
